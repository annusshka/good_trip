...
class FavoriteTourListBloc extends Bloc<FavoriteTourListEvent, FavoriteTourListState> {
  ...
  FavoriteTourListBloc({required this.tourRepository}) : super(FavoriteTourListInitial()) {
  ...
  Future<void> _tourRequested(FavoriteTourListRequested event, Emitter<FavoriteTourListState> emit) async {
    emit(FavoriteTourListLoadInProgress());
    try {
      ...
      emit(const FavoriteTourListLoadFailure(errorMsg: 'Error in favoriteList request.'));
