...
class ExcursionList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<ExcursionCreateBloc, ExcursionCreateState>(
      listener: (BuildContext context1, ExcursionCreateState state) {
        if (state is ExcursionRemovedSuccess) {
          BlocProvider.of<ExcursionCreateListBloc>(context1).add(
            const ExcursionListCreateByActualUserRequested(),
          );
        }
      },
      child: BlocBuilder<ExcursionCreateListBloc, ExcursionCreateListState>(
