...
class AudioPlayerHandlerImpl extends BaseAudioHandler
  ...
  final BehaviorSubject<List<MediaItem>> loadedPlaylist =
      BehaviorSubject.seeded(<MediaItem>[]);
  ...
  Stream<int?> get currentIndex => actualAlbum == loadedAlbum
  ...
  Future<void> startNewPlaylist() async {
    ...
    // _actualPlaylist.addAll(
    //   loadedPlaylist.valueOrNull?.map(_itemToSource).toList() ?? [],
    // );
    actualAlbum = loadedAlbum;
    await _player.setAudioSource(_actualPlaylist);
  }

  Future<void> playNewAudio(int index) async {
    if (!(actualAlbum.isNotEmpty && actualAlbum == loadedAlbum)) {
