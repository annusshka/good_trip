...
class TourCreateListBloc extends Bloc<TourCreateListEvent, TourCreateListState> {
  ...
  Future<void> _tourCreatedListRequested(
    ...
    try {
      final List<AudioTour> tours = await TourService().getCreatedTourListByAdmin(offset: event.offset);
      emit(TourCreateListLoadedSuccess(tourList: tours));
    } catch (_) {
      emit(const TourCreateListLoadFailure(errorMsg: 'Error in created tours request.'));
    }
  }
