...
class ExcursionListBloc extends Bloc<ExcursionListEvent, ExcursionListState> {
  ...
  ExcursionListBloc({required this.excursionRepository})
      : super(ExcursionListInitial()) {
    on<ExcursionListEvent>(
      (event, emit) async {
  ...
  Future<void> _requestExcursionList(
      ExcursionListRequested event, Emitter<ExcursionListState> emit) async {
    emit(ExcursionListLoadInProgress());
    try {
      final List<Excursion> tourList = await excursionRepository
          .getExcursionList(lon: event.lon, lat: event.lat, limit: 10);
